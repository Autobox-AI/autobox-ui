...
export function usePrefetch(config: PrefetchConfig = {}) {
  ...
  const prefetchRunDetails = useCallback(async (runId: string) => {
  ...
  const prefetchRunTraces = useCallback(async (runId: string) => {
    if (!enabled || !runId) return
  ...
  const prefetchRunMetrics = useCallback(async (runId: string) => {
    if (!enabled || !runId) return
...
setInterval(() => {
  ...
  Object.keys(prefetchCache).forEach(key => {
    if (now - prefetchCache[key].timestamp > 300000) { // 5 minutes
