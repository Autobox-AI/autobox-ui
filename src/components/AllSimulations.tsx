...
import { BOOKMARK_TYPES, Simulation } from '@/schemas'
...
type SimulationWithProject = Simulation & { project_name: string; project_id: string }
...
const AllSimulations = ({
  ...
  simulations: SimulationWithProject[]
}) => {
  ...
  const uniqueProjects = Array.from(
    new Set(initialSimulations.map((sim) => sim.project_name))
  ).sort()
        ...
        eventSource.onmessage = (event) => {
          ...
          setSimulations((prevSimulations) =>
            prevSimulations.map((sim) =>
              sim.id === updatedSimulation.id ? { ...sim, ...updatedSimulation } : sim
            )
          )
  ...
  const handleSearch = useCallback(
    async (query: string) => {
      ...
      if (projectFilter !== 'all') {
        filtered = filtered.filter((simulation) => simulation.project_name === projectFilter)
      }
      ...
      if (query.trim()) {
        filtered = filtered.filter(
          (simulation) =>
            ...
            simulation.project_name.toLowerCase().includes(query.toLowerCase())
        )
      }

      setSimulations(filtered)
    },
    [initialSimulations, statusFilter, projectFilter]
  )

  // Add useEffect to refilter when status or project changes
  useEffect(() => {
    handleSearch(searchQuery)
  }, [statusFilter, projectFilter, handleSearch, searchQuery])

  const goToSimulation = (simulation: SimulationWithProject) => {
    router.push(
      `/projects/${simulation.project_id}/simulations/${simulation.id}?projectName=${encodeURIComponent(simulation.project_name)}`
    )
  }
        ...
        <div className="w-full bg-background px-6 pt-6 pb-4 border-b border-zinc-800">
          <div className="max-w-7xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold">All Simulations</h1>
                <p className="text-sm text-zinc-400 mt-1">
                  View and manage all simulations across your organization
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {simulations && simulations.length > 0 ? (
                  simulations.map((simulation) => (
                    <div
                      key={simulation.id}
                      className="group relative bg-zinc-900 rounded-lg border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-all duration-200 h-[320px] flex flex-col"
                    >
                      {/* Project Label */}
                      ...
                      <div className="px-6 pb-4 flex-1">
                        <div className="flex justify-between items-start">
                          <div
                            ...
                            onClick={() => goToSimulation(simulation)}
                          >
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <BookmarkButton
                                ...
                                projectName={simulation.project_name}
                                size="sm"
                                variant="ghost"
                              />
                              <button
                    ...
                    {simulations && simulations.length > 0 ? (
                      simulations.map((simulation) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className="text-blue-400">{simulation.project_name}</span>
                          </td>
                          <td className="px-4 py-3">
